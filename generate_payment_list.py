...
def openpyxl_border_range(ws, start_col, start_row, end_col, end_row ):
	...
	top_lft_corner_cell = [ start_col, start_row ]			# [A1]
	top_rgt_corner_cell = [ end_col,   start_row ]			# [D1]
	bot_lft_corner_cell = [ start_col, end_row   ]			# [A4]
	bot_rgt_corner_cell = [ end_col,   end_row   ]			# [D4]

	top_side_row = start_row								# 1
	bot_side_row = end_row									# 4
	lft_side_col = start_col								# 1
	rgt_side_col = end_col									# 4
...
def group_fill_color( ws, start_col, start_row, end_col, end_row, color ):
	...
	for col_index in range( start_col, end_col + 1  ):
		for row_index in range( start_row, end_row + 1  ):
			ws.cell( row = row_index, column = col_index).fill = PatternFill( start_color = color, end_color = color, fill_type = 'solid' )
...
def make_excel( month, year, list_final_by_nme, list_final_by_opt ,filename ):
	...
	try:
		...
		#ws_a.cell('E2').value = date_start_month.strftime("%Y")
		...
		#ws_c.cell('C2').value = date_start_month.strftime(" %Y")
...
def upload_hr_file( list_fnl ):
	...
	try:
		...
		#for row_index in range (11, 12 + nme_cnt):
		row_index 		= 11
